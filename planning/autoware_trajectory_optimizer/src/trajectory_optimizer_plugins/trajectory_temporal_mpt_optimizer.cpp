...
void TrajectoryTemporalMPTOptimizer::set_up_params()
{
  ...
  mpt_params_.tau_a = get_or_declare_parameter<double>(
    *node_ptr, "trajectory_temporal_mpt_optimizer.acc_time_constant");
  mpt_params_.tau_d = get_or_declare_parameter<double>(
    *node_ptr, "trajectory_temporal_mpt_optimizer.steer_time_constant");
  mpt_params_.max_steer_rate =
    get_or_declare_parameter<double>(*node_ptr, "trajectory_temporal_mpt_optimizer.max_steer_rate");
  mpt_params_.tau_a = std::max(1.0e-4, mpt_params_.tau_a);
  mpt_params_.tau_d = std::max(1.0e-4, mpt_params_.tau_d);
  mpt_params_.max_steer_rate = std::max(1.0e-6, mpt_params_.max_steer_rate);
  mpt_params_.use_previous_solution_warm_start = get_or_declare_parameter<bool>(
    *node_ptr, "trajectory_temporal_mpt_optimizer.use_previous_solution_warm_start");
  ...
  const std::vector<rclcpp::Parameter> & parameters)
{
  ...
  // Applied each solve via setParametersAllStages / setSteerRateLimit (no solver recreate).
  if (update_param(
        parameters, "trajectory_temporal_mpt_optimizer.acc_time_constant", mpt_params_.tau_a)) {
    mpt_params_.tau_a = std::max(1.0e-4, mpt_params_.tau_a);
  }
  if (update_param(
        parameters, "trajectory_temporal_mpt_optimizer.steer_time_constant", mpt_params_.tau_d)) {
    mpt_params_.tau_d = std::max(1.0e-4, mpt_params_.tau_d);
  }
  if (update_param(
        parameters, "trajectory_temporal_mpt_optimizer.max_steer_rate",
        mpt_params_.max_steer_rate)) {
    mpt_params_.max_steer_rate = std::max(1.0e-6, mpt_params_.max_steer_rate);
  }
  update_param(
    parameters, "trajectory_temporal_mpt_optimizer.use_previous_solution_warm_start",
    mpt_params_.use_previous_solution_warm_start);
