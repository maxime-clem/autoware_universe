...
TEST_F(CostFunctionTest, LateralCostsAreMonotonic)
{
  ...
  for (float y_offset = 0.0F; y_offset <= 5.0F; y_offset += 0.5F) {
...
TEST_F(CostFunctionTest, SpeedVsComfortTradeoff)
{
  ...
  auto breakdown_a_speed_priority = evaluateState(state_a, base_u_, 5);
  auto breakdown_b_speed_priority = evaluateState(state_b, base_u_, 5);
  ...
  auto breakdown_a_comfort_priority = evaluateState(state_a, base_u_, 5);
  auto breakdown_b_comfort_priority = evaluateState(state_b, base_u_, 5);
